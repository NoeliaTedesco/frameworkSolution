...
public class Report {
	...
	private static String FILE = DataSetter.configuration.getReportRepository();
	...
	public static void addTestContent(Document document) throws DocumentException {
		for (int i = 0; i < XMLHelper.preObjects.size(); i++) {
			...
			preface.add(new Paragraph(preObjects.get(i).obj.getNombreDePrueba(), catFont));// Will
																							// create:
																							// Report
																							// generated
																							// by:
																							...
																							// _date
			preface.add(new Paragraph(
					ReportConfiguration.testContentExecuteBy + ReportConfiguration.titleAddCreator + ", " + new Date(),
					smallBold));
			addEmptyLine(preface, 1);
			preface.add(new Paragraph(ReportConfiguration.testContentTestDescription, smallBold));
			preface.add(new Paragraph(preObjects.get(i).obj.getNombreDePrueba()));
			addEmptyLine(preface, 1);
			...
			document.add(preface);
			document.add(buildTable(table, preObjects.get(i).obj.getNombreDePrueba(), preObjects.get(i)));
		}
